...
void randR() {
  ...
  randNumber = random(650, 1120);  // degrees of rotation
...
void loop() {
  ...
  if (btt >= 1) {
    ...
    if (((digitalRead(10) == HIGH) and (M == false)) and (analogRead(A1) > r or analogRead(A0) > l) and (digitalRead(8) == HIGH)) {  // ultrasonic true
      motorstop();
      delay(100);  // delay min 200 for completely stop motors and for discharge capacitor on PWB
      search();
      delay(100);
    ...
    if (((digitalRead(8) == LOW) or (digitalRead(10) == HIGH)) and (M == true)) {
      motorstop();
      delay(200);
      propeller();
      motorstop();
